    ...
    if st.sidebar.button("获取最新数据"):
        with st.spinner("正在获取数据..."):
            ...
            if dataset_option == "六滋堂会员日历":
                ...
                base_query = """
                                SELECT  用户昵称,用户手机号,积分,CONCAT_WS(',',添加的企微成员) 添加的企微成员,企微是否加对了团长,历史累计消费,日期,周,
                ...
                if secondary_filter and secondary_filter != "全部门店":
                    query_sql = base_query + f" AND `门店`='{secondary_filter}' GROUP BY 用户昵称,用户手机号,积分,添加的企微成员,企微是否加对了团长,日期,周,累计看播时长,累计领取积分,累计金额,历史累计消费"
                else:
                    query_sql = base_query + " GROUP BY 用户昵称,用户手机号,积分,添加的企微成员,企微是否加对了团长,日期,周,累计看播时长,累计领取积分,累计金额,历史累计消费"
